//Created by Deeksha Khadsare
//Student# 700660741

$(document).ready(function () {

    let data = getAllData();
    let questionId = findGetParameter('id');
	//get perticular question with the help of parameter id in findgetparameter function. 
    let question = data.filter(function (key) {
    ...
    let answers = data.filter(function (key) {
...
function postAnswer(answer, questionId) {
    ...
    let postedDate = new Date().toLocaleDateString() + ": " + new Date().toLocaleTimeString();
    var postData = {
        DT_RowId: uniq,
